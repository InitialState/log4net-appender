...
namespace Log4Net.SyslogLayout.Utilities
{
    /// <summary>
    ///     Provides helper methods for cleaning up string values. It is assumed that the consumer has specific needs for the
    ///     string. For this implmentation this is mainly used to ensure that syslog
    ...
    internal static class PrintableAsciiSanitizer
    {
        ...
        public static string Sanitize(string input, int maxLength, byte[] forbiddenOctets)
        {
            ...
            {
                if (!(b < 33 || b > 126 || Array.FindIndex(forbiddenOctets, item => item == b) != -1))
                {
